    ...
    function bfs(x, y) {
        ...
        while(queue.length !== 0) {
            ...
            for(let i = 0; i < dx.length; i++) {
                ...
                if(newX >= 0 && newY >= 0 && newX < rows && newY < columns && G[newX][newY] === 1 && R[newX][newY] === 0 && crashed === false) {
                    crashed = true;
                    R[newX][newY] = R[cur[0]][cur[1]] + 1;
                    queue.push([newX, newY]);
                }
                
                if(newX >= 0 && newY >= 0 && newX < rows && newY < columns && G[newX][newY] !== 1 && R[newX][newY] === 0) {
                    R[newX][newY] = R[cur[0]][cur[1]] + 1;
                    queue.push([newX, newY]);
                }
            }
